...
export const TETPlugin = async (input: any) => {
  ...
  return {
    ...
    // TET's system prompt, appended to the one opencode builds for every request (measured, 1.18.4
    // host and sbx: see system-prompt.ts). The plugin API has no once-per-session hook; the text is
    // the same every time, so this is the system prompt, not a message.
    "experimental.chat.system.transform": async (_input: any, output: any) => {
      if (!mine || !Array.isArray(output?.system)) return;
