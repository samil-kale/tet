...
export interface UseEscapeOptions {
  /**
   * A selector for an element (monaco's editor root) that handles its own Escape first. Without
   * it the capture-phase listener below closes the dialog on the very Escape the editor needed
   * for its own widget.
   */
  deferWithin?: string;
}
 ...
 * key unhandled (`!event.defaultPrevented`).
 */
export function useEscape(onClose: () => void, options?: UseEscapeOptions): void {
  ...
  // diff dialog re-renders on every diff load and keystroke.
