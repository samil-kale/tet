...
- **Fixes.** On Linux and macOS, quitting through a signal, such as logging out, ends the
  sessions and installs a pending update again. Claude Code's turn ends when it stops, even with
  background tasks still running. Changing a path's access now applies to a sandbox that was
  stopped. Sandboxes are created without sbx's own skills store, which covered the skills folder.
  Deleting a Codex or opencode session that is already gone no longer fails. A dialog shows its
  errors under the field or beside the buttons they concern.
